...
class App extends Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    return images.length !== nextState.images.length ||
      file !== nextState.file ||
  ...
  fetchImages = () => {
    return axios.get('/images').then(res => {
      this.setState({ images: res.data.images, count: res.data.count, file: null });
    });
  };
  ...
  searchChangeHandler = term => {
    ...
    if ( term && term.length > 2 ) {
      ...
      const filteredImages = images.filter(img => img.name.toLowerCase().includes(term.toLowerCase()));

      if ( !filteredImages.length ) {
        return;
      }
      if ( images.length === filteredImages.length && images[0].name === filteredImages[0].name ) {
        return;
      }
