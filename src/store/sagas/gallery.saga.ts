...
export function* fetchImagesSaga() {
  try {
    ...
    yield put(actions.fetchImagesSuccess(result as AxiosResponse) as Action<ActionTypes.FETCH_IMAGES_SUCCESS>);
  } catch (error) {
    yield put(actions.fetchImagesFail(error as AxiosError) as Action<ActionTypes.FETCH_IMAGES_FAIL>);
  }
}
